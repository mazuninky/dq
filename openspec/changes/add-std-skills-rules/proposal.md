...
- **NEW `@std/skills` namespace** с двумя правилами:
  - `skills.frontmatter` — markdown rule, проверяет фронтматтер `SKILL.md`: наличие `name` + `description`, `name` matches `^[a-z0-9][a-z0-9_-]*$`, длина description после whitespace-folding ≤ 1024 (мирроринг лоадера). Fires только когда фронтматтер присутствует И путь матчит `**/SKILL.md` — обычные markdown-документы не цепляет.
  - `skills.evals-schema` — JSON Schema 2020-12 rule поверх `skill/evals/evals.json`. Sidecar `evals.schema.json` лежит рядом с правилом, embedded через тот же механизм, что `@std/jsonschema/helm-values-template.schema.json` и `@std/openapi/openapi-info.schema.json`.
