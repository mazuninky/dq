...
Capability: внутреннее представление документа `Ir` + provenance-канал для propagation source-pointers сквозь read-only трансформации. Фундамент для span-aware lint (`data-query-exec`) и edit-ops vocabulary (`data-query-edit-ops`).
