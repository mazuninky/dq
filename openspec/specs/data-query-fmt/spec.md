...
Defines the `dq fmt` command — the canonicalization verb that re-emits a
file through its format's native writer — together with the global
formatting flags `--sort-keys` (deep alphabetical map-key sort) and
`--indent <N>` (indentation width, JSON/JSONL only). Documents the M4
contract that `fmt` drops comments and anchor metadata as an intentional
canonicalizer, while `set`/`del`/`patch`/`merge` keep the M2 textual-edit
splice path that preserves them. Specifies
which formats honor each flag, the `--check` idempotency gate semantics
used by pre-commit hooks, and the M4 anti-scope around `--quote-style` /
`--flow-style` / `--strip-comments` (deferred until a comment-preserving
emitter lands).
