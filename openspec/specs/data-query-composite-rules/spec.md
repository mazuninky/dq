# data-query-composite-rules Specification

## Purpose

Defines `dq-exec` cross-format composite-rule support: the `Rule.check.extract` + `Rule.check.nested` pair, the contract for the `extract` jq expression, parse-failure semantics, coordinate projection from inner-document space back to outer-file coordinates via anchor + inline-offset, and the recursion depth bound (M11).
