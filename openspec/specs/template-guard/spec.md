# template-guard Specification

## Purpose

Defines how `dq` recognises files that contain Go-template syntax (Helm,
Argo, Flux) or GitHub Actions expression syntax before any format parser
runs, and the contract for the two opt-in escape-hatches that let users
operate on such files anyway. The guard exists because templated files
are not valid YAML/JSON/TOML and would crash the parser with a confusing
diagnostic; surfacing a structured `Error::TemplatedFile` up front gives
agents and humans a clear next step instead of a parser stack trace.
Consumers: every `dq` command that reads a file (`get`, `paths`, `set`,
`del`, `convert`, `validate`).
