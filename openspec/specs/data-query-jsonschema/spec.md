# data-query-jsonschema Specification

## Purpose

Defines `dq-exec` JSON Schema 2020-12 rule support: the `Rule.check.schema` (inline) and `Rule.check.schema_file` (sibling-path) variants, validator compilation lifecycle, mapping of `instancePath` to RFC 6901 `Pointer`, security restrictions on `$ref`, and the reference `@std/jsonschema` ruleset (M11).
