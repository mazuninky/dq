# data-query-bulk Specification

## Purpose

Defines the contract for bulk / multi-file operations across every write
command (`set`, `del`, `patch`, `merge`, `convert`). Covers `globset`-style
glob expansion, the `--continue-on-error` partial-failure mode, the
`--parallel <N>` rayon thread-pool driver, the `--check` idempotency gate
used by pre-commit hooks, and the partial-success exit-code mapping
(`WRITE_FAILED = 7`). Single-file invocations remain byte-identical to the
M2 baseline; the bulk surface is purely additive.
